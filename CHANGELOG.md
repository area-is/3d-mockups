...
- **`MilkCartonMockup` / `MilkCarton`** — a gable-top beverage carton
  ...
  poly-coated board walls, the roof folded up to a ridge with the ear folds
  closing both ends, the sealed fin, and a screw cap on the front roof panel.
  Live surfaces on all four walls plus both roof panels — the cap rides over
  the front one the way a real spout rides over the print. Measures as
  `mockupInfo('milkCarton')`.
